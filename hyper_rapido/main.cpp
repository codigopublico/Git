...
pnodo lista::Ultimo(){
    std::cout << "Se esta ejecutado ultimo\n";
    pnodo nodo = actual;
    int a = 0;
    while(nodo && nodo->siguiente){ 
        nodo = nodo->siguiente;
        std::cout << "Siguiente pro favor\n" << "  " <<nodo->peli.e << "  " <<nodo->lib.e << " " <<nodo->disc.e << "  " << nodo->disc.titulo << "  ";
        //if(nodo->disc.e && nodo->lib.e && nodo->peli.e) break;
        if(!nodo) break;
    }
    return nodo;
}
...
void lista::Insertar(discos ID){
   ...
   if(actual == NULL) actual = Nodo;
    ...
    while(actual->siguiente){
        ...
        std::cout << "Los nodos son estos\n";
        std::cout << actual->disc.titulo << "\n";
    }
...
void lista::Mostrar()
{
   int n = 0;
   std::cout << "Se esta ejecutado ultimo()";
   pnodo nodo = Ultimo();//esto es lo que no fuciona 
   std::cout << "Porque no hace nada\n";
   //pnodo nodo = actual;
   if(nodo){
   do {
      //cout << nodo->valor << "-> ";
...
void menu1(){
    ...
    do{
        ...
        if(aux == "3"){
            ...
            if(n == 0){
                std::cout << "Se esta ejecutado uno----------------\n";
            PUlist.Mostrar();
            }else{
                std::cout << "Se ejecuta esta------------------\n";
                PUlist.Mostrar(n);
            }
        }
