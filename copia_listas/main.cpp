...
struct dato {
  char nombre[20];
  char autor[20];
  int nivel; // esto se ha de implementar tengo que printear todo el programa
  int duracion;
};
...
//hasta aqui bien se definen los datos.
//i las funciones del la estructura de datos.
...
struct lista *insertaordenado(struct lista *l, struct dato x, int (*f)(struct dato a, struct dato b)) {
  ...
  ind = 0;
  while (p != NULL && ind == 0) {
    if ((*f)(x,p->datos) <= 0) /* aquí hay que insertar, aqui es donde se implementa la funcion */
      ind = 1;
    else {
      ant = p;
      p = p->sig;
...
struct lista *elimina(struct lista *p, struct dato x, int (*f)(struct dato a, struct dato b)) {
  ...
  if (p == NULL) /* no hay nada que borrar */
    return p;
  /* compara el dato */
  cond = (*f)(x,p->datos);//esto es una coparacion que se base en la el paso por puntero de una funcion.
...
int alfabetica(struct dato x, struct dato y) {
  ...
  return i;
}//Esta se llamara compdura en mi programa, se podria usar un boleano. ?
...
void procesanodos(struct longydir *lg, int nodoini, int nodofin, int indfinal) {
  ...
  for (i = nodoini; i <= nodofin; ++i) {
    m = lg[i].lgmax;
    centracad(lg[i].p->autor,m + 2,auxiliar);
    printf("|%s|",auxiliar);
    ind = 1; /* poner los espacios */
    if (i == nodofin) {
      if (indfinal == 0)
...
struct dato tomadatos(void) {
  ...
  std::cin >> dura;
  printf("Introduce el nivel de la cancion:     ");
  std::cin >> nivel;
  x.duracion = dura;
  x.nivel = nivel;
...
int main (int argc, char *argv[]) {
  ...
  while (1) {
    ...
    case 1: /* Insertar */
      ...
      l = insertaordenado(l,x,alfabetica);
       break;
    case 2: /* Borrar */
      ...
      /*
	aunque no hace falta, asignar los demás campos de x,
	por si alguien hace algo que no debe.
      */
      x.autor[0] = '\0';
      //x.nombre[0] = '\0';
      x.duracion = 0;
